...
| Daemon restarts every ~2 min | Watchdog: daemon stopped sending `WATCHDOG=1` (main loop wedged) | Working as designed since 2026-07-04: the unit is `Type=notify` + `WatchdogSec=120` and the daemon pings once per hot cycle (`inverter_bridge/sdnotify.py`). If it restarts in a loop, check the journal for what blocks the main loop *before* touching the watchdog |
