...
- **Auditoría completa código+host:** [docs/2026-07-04_revision_codigo-y-host.html](docs/2026-07-04_revision_codigo-y-host.html). Ronda 1 (commit `1c3ecef`): A1/A2/M3/M5/M7/M8/M1-min/B1/B8/B9 + watchdog + deploy.sh. Ronda 2 (commit `4ada645`): M6 (packs BMS congelados expiran de los agregados, `bms.stale_pack_timeout_s`), M4 (`inverter_<N>_status` online/offline completo con discovery), B2 (`inverter_N_fault_bits` hex del bloque 0x0204 + poda de 5 bloques cold muertos), M1-completo (worker colgado no se re-encola), B4/B5/B7. Tests en `tests/test_audit_fixes.py`. **Backlog restante: solo M2** (matching de respuesta por count vs chatter del LCD — requiere capturar fixtures reales del bus antes de tocar; ver informe).
