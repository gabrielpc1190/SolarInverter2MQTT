...
- **Auditoría completa código+host:** [docs/2026-07-04_revision_codigo-y-host.html](docs/2026-07-04_revision_codigo-y-host.html) — **23/23 hallazgos RESUELTOS**. Ronda 1 (`1c3ecef`): A1/A2/M3/M5/M7/M8/M1-min/B1/B8/B9 + watchdog + deploy.sh. Ronda 2 (`4ada645`): M6 (packs BMS congelados expiran, `bms.stale_pack_timeout_s`), M4 (`inverter_<N>_status` completo), B2 (`inverter_N_fault_bits` + poda de 5 bloques muertos), M1-completo, B4/B5/B7. Ronda 3 / M2 (`serial_io`): matching de respuesta por dirección — `responses_with_context()` + `select_response()` (3 niveles: dirección exacta → huérfana → count). Tests en `tests/test_audit_fixes.py`.
- **Bus serial = bus compartido con el LCD** (captura real 2026-07-04, `tools/capture_fixtures.py`, fixtures `bus_capture_*_m2_*.hex`): el LCD es un segundo maestro Modbus; cada respuesta viene precedida por su request (que lleva la dirección), pero **nuestra propia request NO hace eco en RX** (nuestra respuesta queda "huérfana"). Hallazgo clave: **todos los tamaños de bloque que pedimos son únicos en este bus**, así que la colisión teórica de M2 no se manifiesta hoy — el matching por count era seguro; el fix por dirección es defensa a futuro. `select_response` tiene fallback a count (nivel 3) para no regresar a timeouts.
- **`query()` devuelve apenas la respuesta está completa** (no espera a llenar buffer): drena `in_waiting` y re-matchea tras cada chunk. El bucle viejo de cap-fijo esperaba `expected+32` bytes y, sin un hueco de silencio en el chatter, eso solo pasaba al deadline → cada poll tomaba los 1.5s completos (medido 1522ms/bloque, ciclo 6.3s > intervalo 3s). Con el fix: ~150ms/bloque, ciclo ~640ms.
- **Journal del host SÍ es persistente** (corrección al hallazgo H6 del informe): `/var/log/journal → /var/log.hdd/journal` (eMMC) por diseño de armbian-ramlog; sobrevive reboots, cap 20M.
